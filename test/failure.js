...
describe('sending notification to an invalid channel', function () {
	it('fails', function (done) {
		...
		wns.send(invalidChannel, validTile, 'wns/tile', options, function (error, result) {
			callback(error, result, done, nockFile, mockScopes, function () {
				...
				assert.equal(typeof error.newAccessToken, 'string', 'Access token had been obtained');
			});
		});
	});
});

describe('using invalid application credentials', function () {
	it('fails', function (done) {
