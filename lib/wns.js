...
var createTemplate = function (type, name, imageCount, textCount) {
	var template = '<' + type + '><visual><binding template="' + name + '">';
	for (var i = 0; i < imageCount; i++)
		template += '<image id="' + (i + 1) + '" src="%s" alt="%s"/>';
	for (var i = 0; i < textCount; i++)
		template += '<text id="' + (i + 1) + '">%s</text>';
	template += '</binding></visual></' + type + '>';
...
exports.send = function () {
	...
	if (callback && typeof callback !== 'function')
		throw new Error('The fifth parameter, if specified, must be the callback function.');
		...
		exports['send' + templateName] = function () {
			...
			if (typeof arguments[0] === 'object') {
				...
				for (var i = 1; i <= textCount; i++) {
					...
					if (typeof text !== 'undefined' && typeof text !== 'string')
						throw new Error('The text' + i + ' property of the payload argument must be a string.');
			...
			if (params.length !== (textCount + imageCount * 2))
				throw new Error('The ' + templateName + ' WNS notification type requires ' + (textCount + imageCount * 2)
					+ ' text parameters to be specified (' + imageCount + ' images that require href and alt text each, and '
					+ textCount + ' text fields), while only ' + params.length + ' have been provided.');

			// XML escape text parameters and construct payload from template

			var xmlEscapedParams = [ template ];

			params.forEach(function (text) {
			    xmlEscapedParams.push(text.replace(/&/g, '&amp;')
	               .replace(/</g, '&lt;')
	               .replace(/>/g, '&gt;')
	               .replace(/"/g, '&quot;'));
			});
