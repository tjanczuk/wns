...
var createTemplate = function (type) {
	...
	return '<' + type + '%s><visual%s>%s</visual>%s%s</' + type + '>';
};
...
var createActions = function (type, options) {
	if (type !== 'toast' || !options.actions || !options.actions.length)
		return '';

	var actions = '';
	for (var i=0 ; i<options.actions.length ; i++) {
		actions += `<action`;
		if (options.actions[i].content) {
			actions += ' content="' + xmlEscape(options.actions[i].content) + '"';
		}
		if (options.actions[i].arguments) {
			actions += ' arguments="' + xmlEscape(options.actions[i].arguments) + '"';
		}
		if (options.actions[i].imageUri) {
			actions += ' imageUri="' + xmlEscape(options.actions[i].imageUri) + '"';
		}
		actions += ' />';
	}
	...
	(function () {
		...
		exports['create' + templateName] = function () {
			...
			var params = [ 
				...
				createActions(type, options)            // optional toast actions XML
			];
