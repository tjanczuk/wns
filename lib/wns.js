        ...
        res.on('end', function () {
            if (!completed) {
                ...
                if (res.statusCode === 200) {
                    ...
                    var error = new Error('Unable to obtain access token for WNS. HTTP status code: ' + res.statusCode + '. HTTP response body: ' + body);
                    error.statusCode = res.statusCode;
                    error.headers = res.headers;
        ...
        res.on('end', function () {
            if (!completed) {
                ...
                if (context.callback) {
                    ...
                    if (res.statusCode === 200 && res.headers['x-wns-status'] === 'received') {
                        ...
                        var result = new Error(responseCodes[res.statusCode] || ('Windows Notification Service returned HTTP status code ' + res.statusCode + ' with x-wns-status value of ' + res.headers['x-wns-status']));
...
var sendTileOrToast = function () {
    ...
    var options;
    if (arguments.length > 0 && typeof arguments[arguments.length - 1] === 'object' && arguments[arguments.length - 1].type === undefined) {
...
exports.send = function () {
    ...
    if (!types.some(function (item) { return type === item; }))
        throw new Error('The type parameter must specify the notification type. The value of ' + type + ' is not in the set of valid value types: ' + JSON.stringify(types));

    if (typeof options.client_secret !== 'string' || typeof options.client_id !== 'string')
        throw new Error('The options.client_id and options.client_secret must be specified as strings or the WNS_CLIENT_ID and WNS_CLIENT_ID environment variables must be set.');
...
exports.sendBadge = function () {
    ...
    if (!isNaN(realValue)) {
        ...
        throw new Error('The badge value must be either an integer greater than or equal to 0 or one of ' + JSON.stringify(badges));
    }
...
var createAudio = function (type, options) {
    ...
    if (typeof options.audio.src === 'string') {
        if (!audioSources.some(function (src) {
            ...
            throw new Error('The options.audio.src must be a string value from the following set: ' + JSON.stringify(audioSources));
        }
    ...
    if (options.audio.src) {
        result += ' src="' + (options.audio.src.indexOf(audioSourcePrefix) === 0 ? options.audio.src : audioSourcePrefix + options.audio.src) + '"';
    }
...
var createToastAttributes = function (type, options) {
    ...
    if (typeof options.duration === 'string') {
        if (!durations.some(function (duration) {
            ...
            throw new Error('The options.duration must be a string value from the following set: ' + JSON.stringify(durations));
        }
...
var createVisualAttributes = function (type, options) {
    ...
    if (typeof options === 'object') {
        ['lang', 'baseUri', 'branding', 'addImageQuery', 'version'].forEach(function (prop) {
            if (typeof options[prop] === 'string' || typeof options[prop] === 'boolean' || typeof options[prop] === 'number') {
    ...
    (function () {
        ...
        exports['create' + templateName + 'Binding'] = function () {
            ...
            if (params.length !== (textCount + imageCount * 2)) {
                throw new Error('The ' + templateName + ' WNS notification type requires ' + (textCount + imageCount * 2) + ' text parameters to be specified (' + imageCount + ' image(s) that require href and alt text each, and ' + textCount + ' text field(s)), while only ' + params.length + ' parameter(s) have been provided.');
            }
...
var createTileOrToast = function (/* type, [payload]+, [options] */) {
    ...
    for (var i = 0; i < arguments.length; i++) {
        if (typeof arguments[i] === 'string') {
            ...
            throw new Error('Unsupported type of argument: ' + typeof arguments[i]);
        }
    }
