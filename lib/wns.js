...
var localAudioSourcePrefix = 'ms-appx:///';
...
var createAudio = function (type, options) {
	...
	if (typeof options.audio.src === 'string') {
		...
		}) && !(localAudioSourcePrefix === options.audio.src.substr(0, localAudioSourcePrefix.length)))
            throw new Error('The options.audio.src must be a string value from the following set: '
                + JSON.stringify(audioSources) + ' OR a path to local file with ' + localAudioSourcePrefix);
	}
	...
	if (options.audio.src)
		result += ' src="' 
				+ (options.audio.src.indexOf(audioSourcePrefix) === 0 ? options.audio.src :
                    options.audio.src.indexOf(localAudioSourcePrefix) === 0 ? options.audio.src : audioSourcePrefix + options.audio.src)
				+ '"';
